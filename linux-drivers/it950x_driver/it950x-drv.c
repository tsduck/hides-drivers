...
)
{
	...
	if(error != Error_NO_ERROR) {
		pdc->deviceType = EagleUser_DEVICETYPE;
		pdc->isUsingDefaultDeviceType = true;
		deb_data("EagleUser_getDeviceType\n\t- No EEPROM or EEPROM Read no Value.\n");
		deb_data("\t- Load Device Type: %d from default\n", pdc->deviceType);
	} else {
		pdc->isUsingDefaultDeviceType = false;	
		deb_data("EagleUser_getDeviceType\n\t- Device Type: %d load from EEPROM\n", pdc->deviceType);
	}
	...
	if(EagleUser_setSystemConfig((Modulator*) &pdc->modulator, syscfg) != 0)
	...
	if((temp == 1) && (error == ModulatorError_NO_ERROR)){  // eeprom
		...
		error = IT9507_readRegister ((Modulator*) &pdc->modulator, Processor_LINK, EEPROM_SLAVEI2CADDRESS, &temp);
		...
		if(IT9507_setSlaveIICAddress((Modulator*) &pdc->modulator, temp) != 0)
			deb_data("- IT9507_setSlaveIICAddress fail -\n");	
		else
			deb_data("- IT9507_setSlaveIICAddress:%X ok -\n", temp);
	} else {    // no eeprom
		/* Read Default */
		if(IT9507_setSlaveIICAddress((Modulator*) &pdc->modulator, SLAVE_DEMOD_2WIREADDR) != 0)
			deb_data("- IT9507_setSlaveIICAddress fail -\n");	
		else
			deb_data("- IT9507_setSlaveIICAddress as default: %X ok -\n", SLAVE_DEMOD_2WIREADDR);
	}
	...
	if(pdc->modulator.booted) //from Standard_setBusTuner() > Standard_getFirmwareVersion()
    	{
        	...
        	if(error) deb_data("DRV_Initialize : IT9507_getFirmwareVersion : error = 0x%lx\n", error);
        	...
        	if(cmdVersion != fileVersion)
        	{
            		...
            		if(error) 
            		{
                		deb_data("IT9507_reboot : error = 0x%lx\n", error);
                		return error;
            		}
            		else {
    ...
    {		
		switch(streamType_t)
		{
			case StreamType_DVBT_DATAGRAM:
				...
				if (error) deb_data("IT950x_initialize _Device initialize fail : 0x%lx\n", error);
				...
				if(pdc->modulator.slaveIICAddr == 0x38 || pdc->modulator.slaveIICAddr == 0x3A || 
					...
					if (error) deb_data("    Device RX no initialized\n");
						...
						switch(chip_version) {
						case 1: 	/* Omega I */
							error = OMEGA_supportLNA((Demodulator*) &pdc->demodulator ,OMEGAI_RX_LNA_TUNER_ID_SUPPORT_TYPE);
							if (error) deb_data("OMEGA_supportLNAfail : 0x%lx\n", error);
							else deb_data("    OMEGAI: OMEGA_supportLNA Ok. Tuner id Support Type: 0x%X\n", OMEGAI_RX_LNA_TUNER_ID_SUPPORT_TYPE);	
							break;
						case 2: 	/* Omega II */
							error = OMEGA_supportLNA((Demodulator*) &pdc->demodulator ,OMEGAII_RX_LNA_TUNER_ID_SUPPORT_TYPE);
							if (error) deb_data("OMEGA_supportLNAfail : 0x%lx\n", error);
							else deb_data("    OMEGAII: OMEGA_supportLNA Ok. Tuner id Support Type: 0x%X. Support TPS Decryption\n", OMEGAII_RX_LNA_TUNER_ID_SUPPORT_TYPE);
							break;
						default:
							deb_data("    Unknown chip version!!. No RX chip!\n");							
							error = Error_INVALID_TUNER_TYPE;
							break;
						}
						if(error == Error_NO_ERROR) {
							error = Demodulator_initialize ((Demodulator*) &pdc->demodulator , StreamType_DVBT_DATAGRAM); 
							if (error) deb_data("Demodulator no initialized\n");
			...
			case StreamType_DVBT_SERIAL:
				...
				if (error) deb_data("IT9507_initialize _Device initialize fail : 0x%lx\n", error);
				else deb_data("    Device initialize TX Ok\n");
		...
		deb_data("EagleUser_getTsInputType fail");	
		return error;
	}
...
Dword DRV_SetDCCalibrationTable(void* handle) {
	...
	//-------------set DC Calibration table
     error = IT9507_readRegister ((Modulator*) &pdc->modulator, Processor_LINK, 0x4979, &eeprom);//has eeprom ??
     if (error) goto exit;
     error = IT9507_readRegister ((Modulator*) &pdc->modulator, Processor_LINK, 0x49D6, &DCcalibration);//has DCcalibration ??
     if (error) goto exit;
     if((eeprom ==1) && ((DCcalibration & 0x80) == 0x80)){
		error = IT9507_readRegisters ((Modulator*) &pdc->modulator, Processor_LINK, 0x49D6, 12, DCvalue);//DC calibration value
		if (error) goto exit;
		error = IT9507_readRegisters ((Modulator*) &pdc->modulator, Processor_LINK, 0x49CC, 5, OFS_I_value);//OFS_I calibration value
		if (error) goto exit;
		error = IT9507_readRegisters ((Modulator*) &pdc->modulator, Processor_LINK, 0x49E2, 5, OFS_Q_value);//OFS_Q calibration value
		if (error) goto exit;
		...
		if(IT9507_setDCtable((Modulator*) &pdc->modulator, pdc->dcInfo) == 0)
...
DWORD Device_init(struct usb_device *udev, PDEVICE_CONTEXT PDC, Bool bBoot)
{
	...
	deb_data("        EAGLE_FW_RELEASE_LINK_VERSION: %d.%d.%d.%d\n", DVB_LL_VERSION1, DVB_LL_VERSION2, DVB_LL_VERSION3, DVB_LL_VERSION4);
	deb_data("        EAGLE_FW_RELEASE_OFDM_VERSION: %d.%d.%d.%d\n", DVB_OFDM_VERSION1, DVB_OFDM_VERSION2, DVB_OFDM_VERSION3, DVB_OFDM_VERSION4);	
#else
	deb_data("        EAGLE_FW_RELEASE_LINK_VERSION: v0_0_0_0\n");	
	deb_data("        EAGLE_FW_RELEASE_OFDM_VERSION: v0_0_0_0\n");		
#endif
